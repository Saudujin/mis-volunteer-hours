...
| B | نوع الإنجاز | اسم الإنجاز |
| C | الساعات | عدد الساعات لهذا الإنجاز |
...
**الصف الأول (Header):**
```
الرقم الجامعي | نوع الإنجاز | الساعات | رابط الإثبات | التاريخ | معتمد | معتمد بواسطة
```
...
```
=TEXTJOIN(" ، ", TRUE, FILTER(Requests!B:B&" ("&Requests!C:C&")", (Requests!A:A=A2)*(Requests!F:F="TRUE")))
```

**شرح المعادلة:**
- `FILTER` - تصفية الطلبات المعتمدة لهذا العضو
- `TEXTJOIN` - دمج النتائج بفاصلة
- النتيجة مثال: `حضور يوم تعريفي (1) ، لايك تغريدة (0.5)`

**معادلة بديلة (إذا لم تعمل الأولى):**
```
=IFERROR(TEXTJOIN(" ، ", TRUE, FILTER(Requests!B:B&" ("&Requests!C:C&")", (Requests!A:A=A2)*(Requests!F:F="TRUE"))), "")
```
